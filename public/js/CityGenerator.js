		...
		for( var blockX = 0; blockX < nBlockX; blockX++){
			for( var blockZ = 0; blockZ < nBlockZ; blockZ++){	
				...
				for(var i = 0; i < carLightsDensityD+1; i++){
					...
					position.z = (i/carLightsDensityD-0.5)*(blockSizeZ-roadD)*Math.random();
		...
		for( var blockZ = 0; blockZ < nBlockZ; blockZ++){
			for( var blockX = 0; blockX < nBlockX; blockX++){
					for( var i = 0; i < blockDensity; i++){
						for(var j = 0; j < blockDensity; j++){
						if (Math.random() < .1) continue;
						// set position
						buildingMesh.position.x = 0.5*(blockSizeX-buildingMaxW-roadW-sidewalkW)
						buildingMesh.position.z = 0.5*(blockSizeZ-buildingMaxD-roadD-sidewalkD)

						// add position for the blocks
						buildingMesh.position.x	+= (blockX+0.5-nBlockX/2)*blockSizeX - buildingMaxW*i - 2;
						buildingMesh.position.z	+= (blockZ+0.5-nBlockZ/2)*blockSizeZ - buildingMaxD*j - 4;
